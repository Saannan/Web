...
async function eggySrc(film) {
const ress = await axios.get(`https://egydead.center/?s=${film}`);
const $ = cheerio.load(ress.data);
const movieItems = [];
$('li.movieItem').each((i, element) => {
const title = $(element).find('.BottomTitle').text();
const link = $(element).find('a').attr('href');
const img = $(element).find('img').attr('src');
movieItems.push({ title, link, img });
});
 return {
result: movieItems, 
request_at: new Date()
...
async function eggyDetail(url) {
const ress = await axios.get(url);
const $ = cheerio.load(ress.data);
const filmTitle = $("div.singleTitle em").text().trim(); 
const postDate = $("div.postDate").text().trim(); 
const filmDescription = $(".singleStory").text().trim(); 
const story = $(".extra-content p").text().trim(); 
const thumbnail = $(".single-thumbnail img").attr("src"); 
const trailerButton = $(".trailerBtn");
let trailerLink = null;
if (trailerButton.length > 0) {
const iframeSrc = $("div#trailerPopup iframe").attr("src");
if (iframeSrc) {
trailerLink = iframeSrc;
}}
const filmData = {
title: filmTitle,
postDate: postDate,
...
async function hoaxSrc(category) {
const baseUrl = 'https://infohoax.bondowosokab.go.id';
const ress = await axios.get(`${baseUrl}/pencarian?cari=${category}`);
const $ = cheerio.load(ress.data);
const hoaxData = [];
...
async function hoaxDetail(url) {
const ress = await axios.get(url);
const $ = cheerio.load(ress.data);
const title = $('h2.font-weight-semi-bold a').text().trim();
const imageUrl = $('.post-image img').attr('src');
const day = $('.post-date .day').text().trim();
const month = $('.post-date .month').text().trim();
const content = $('.post-content p').map((index, element) => $(element).text().trim()).get().join('\n');
const sourceLinks = $('ul li a')
.map((index, element) => {
const href = $(element).attr('href');
if (href.includes('kompas.com') || href.includes('kominfo.go.id')) {
return href;
}
return null;
})
...
return {
title,
imageUrl: `https://infohoax.bondowosokab.go.id${imageUrl}`,
date: fullDate,
content,
sourceLinks
}}

module.exports = { komiku, mcpedl, CarbonifyV1, CarbonifyV2, imagetohd, remini, recolor, dehaze, removeBg, Andro1, animeSrc, Cerpen, Apkpure, liteApks, eggySrc, eggyDetail, hoaxSrc, hoaxDetail }